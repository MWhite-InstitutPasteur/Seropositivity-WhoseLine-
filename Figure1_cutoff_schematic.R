...
GMM_model <- stan_model(file="Figure1_GMM_unknown_Ab_data.stan")
...
{
	plot(x=-1, y=-1, 
	xlim=c(0,N_sam), ylim=range(GMM_chains[,j]),
	...
	{
		points(x = 1:N_sam, 
                   y = GMM_chains[1:N_sam + (m-1)*N_sam,j], 
			 type='l', col=chain_cols[m], cex=0.25 )
	}


	DEN_1 <- density( GMM_chains[1:N_sam ,j] )
	...
	for(m in 2:N_chains)
	{
		DEN_m <- density( GMM_chains[1:N_sam + (m-1)*N_sam,j] )
